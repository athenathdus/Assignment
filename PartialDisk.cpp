...
void MyDisplay() {
	...
	glLoadIdentity();
	glEnable(GL_LIGHTING);
	//gluPartialDisk(qobj, 1.0, 2.5, 26, 13,0,100); 가운데가 비어있는 partialdisk
	gluPartialDisk(qobj, 0.0, 2.5, 26, 13, 0, 100); //안쪽 반지름이 0일경우, 비어있지 않은 원이 나오며, 안쪽반지름이 0보다 클경우 가운데가 비어있게 된다. 
	glutSwapBuffers();
...
int main(int argc, char** argv) {
	...
	glutCreateWindow("PartialDisk");  // 창 이름을 PartialDisk 로 지정 
